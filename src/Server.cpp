...
int Server::new_connection(struct epoll_event &event) {
  ...
  char hostname[128];
  int returngetname;
  if ((returngetname = getnameinfo((struct sockaddr *)&in_addr, sizeof(in_addr),
...
ssize_t Server::ser_recv(struct epoll_event &event)
{
	...
	std::cout << "Client_FD[" << event.data.fd << "] wrote'" << message << "'"
			  << std::endl;
...
void Server::generate_socket() {
  ...
  _sock_serv.sin_port =
      htons(_port); // on convertit le port en ordre octet réseau
