...
CLIENTS Server::getClients() const { return (_map_client); }

CHANNELS Server::getChannels() const { return (_map_channel); }
...
int Server::new_connection(struct epoll_event &event) {
    ...
    if (fcntl(infd, F_SETFL, O_NONBLOCK) < 0)
        throw std::runtime_error("Erreur lors de la configuration du socket en mode non-bloquant.");
...
ssize_t Server::ser_recv(struct epoll_event &event) {
    ...
    while (getline(messageStream, line, '\n')) {
        ...
        std::cout << "Client_FD[" << event.data.fd << "] wrote'" << line << "'" << std::endl;
...
void Server::generate_socket() {
    ...
    _sockfd = socket(AF_INET, SOCK_STREAM, 0); // AF_INET -> on utilise IPv4, SOCK_STREAM -> socket de type
    // flux (TCP), 0 pour mettre le protocole par defaut (IPv4)
    if (_sockfd == -1) {
        throw std::runtime_error("Erreur lors de la génération du socket");
    }
    if (fcntl(_sockfd, F_SETFL, O_NONBLOCK) < 0) { // fcntl fonction modifiant les attribut socket, F_SETFL pour set un
    ...
    if (listen(_sockfd, _sock_serv.sin_port) < 0) {
        throw std::runtime_error("Erreur lors de la mise en mode d'écoute du socket.");
    }
