...
Client::Client(std::string hostname, int fd, Server *s) : _hostname(hostname),
														  _fd(fd), _serv(s)
{
	_nickname = "undefined";
}
...
void Client::send_msg(std::string msg, std::string sender)
{
	...
	while (pos < msg.length())
	{
		...
		{
			formattedMsg += msg.substr(pos, newlinePos - pos + 1) +
							std::string(16, ' ');
			pos = newlinePos + 1;
		} else
		{
	...
	std::string format = "[" + colorCode + "%-12s" + RESET + "]: %s\r\n";
	snprintf(messageWithSender, sizeof(messageWithSender), format.c_str(),
			 sender.c_str(), formattedMsg.c_str());

	if (send(_fd, messageWithSender, strlen(messageWithSender), 0) < 0)
	{
		throw std::runtime_error(
				"Erreur lors de l'envoi du message au client.");
	}
}
...
{
	outFile << BOLDWHITE << "Nickname: " << RESET << client.getNickname()
			<< std::endl;
	outFile << BOLDWHITE << "Realname: " << RESET << client.getRealname()
			<< std::endl;
	outFile << BOLDWHITE << "Hostname: " << RESET << client.getHostname()
			<< std::endl;
	outFile << BOLDWHITE << "File Descriptor: " << RESET << client.getFd()
			<< std::endl;
